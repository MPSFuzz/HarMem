...
from src.harness_analysis import analyze_harness_structure
from src.cve_helper.constraint_model import build_generation_constraints
from src.harness_class.harness_structural_refine import structural_refine_harness
...
def _process_root_api(root_api, llm_global: LLM, call_chain, cve_hints_obj = None, max_fix = 3):
    ...
    llm.get_cve_hints(cve_hints_obj)
    ...
    if cve_hints_obj is not None:
        try:
            constraints = build_generation_constraints(plan, phase_A_context, cve_hints_obj)

            analysis_result = analyze_harness_structure(code = h.code, target_api=llm.target_func, constraints=constraints)

            refine_result = structural_refine_harness(
            llm=llm,
            h=h,
            plan=plan,
            ...
            logger.info(
            f"[structural-refine-result] root_api={root_api}, accepted={refine_result['accepted']}")
        
        except Exception as e:
            logger.warning(f"Structural analyzer/refine failed for root api {root_api}: {e}")
