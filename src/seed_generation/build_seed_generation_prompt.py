...
def build_seed_generation_prompt(batch: Batch,
    ...
    lines.append(
        ...
        "- Only use stdlib modules (struct, random, os, sys, string, itertools, subprocess, etc.).\n"
        "- Print the count of generated seeds to stdout on success.\n"
        "- NOT use network or filesystem outside the output directory.\n"
    ...
    if batch.lib_name and "tiff" in batch.lib_name.lower():
        lines.append(
            "███ FORCED for libtiff ███\n"
            "DO NOT write code that constructs TIFF files from scratch using "
            "struct/zlib/bytearray — it will FAIL.\n"
            "\n"
            "TWO-STEP APPROACH (MUST follow):\n"
            "\n"
            "Step 1 — Use PIL to create a SIMPLE base TIFF from a template image:\n"
            "  from PIL import Image\n"
            "  img = Image.open('/root/auto_harness/src/seed_generation/template_images/small_rgb.png')\n"
            "  img = img.crop((0, 0, width, height))       # vary dimensions\n"
            "  img.save(base_path, format='TIFF')           # plain TIFF, NO compression/tile args\n"
            "\n"
            "Step 2 — Use compiled tiffcp tool to apply compression/tiling:\n"
            "  import subprocess, os\n"
            "  env = os.environ.copy()\n"
            "  env['LD_LIBRARY_PATH'] = os.environ.get('LD_LIBRARY_PATH', '')\n"
            "  env['PATH'] = '/root/experiment/' + lib_name + '_CVE-XXXX/tools:' + env.get('PATH', '')\n"
            "  subprocess.run(['tiffcp', '-c', 'lzw', base, out], env=env, check=True)  # compression\n"
            "  subprocess.run(['tiffcp', '-t', '-w', '64', '-l', '64', base, out], env=env, check=True)  # tiled\n"
            "\n"
            "tiffcp options: -c (lzw/zip/pixarlog/packbits/...), -t (tile), -w (tile_w), -l (tile_h),\n"
            "  -r (rows/strip), -s (strip mode), -8 (BigTIFF)\n"
            "\n"
            "WARNING — PIL DOES NOT SUPPORT: pixarlog compression, tiled TIFF, BigTIFF — "
            "must use tiffcp for those.\n"
            "\n"
            "If the harness appends extra data after TIFF bytes (row/col etc.), use 'ab' mode:"
            "  with open(out_path, 'ab') as f: f.write(struct.pack('<I', val))"
        )
    elif batch.lib_name in ("libpng", "libjpeg", "libwebp"):
        lines.append(
            "███ FORCED for image libraries ███\n"
            "DO NOT construct binary image files from scratch — use PIL/Pillow instead.\n"
            "\n"
            "  from PIL import Image\n"
            "  img = Image.open('/root/auto_harness/src/seed_generation/template_images/small_rgb.png')\n"
            "  img = img.crop((0, 0, width, height))\n"
            "  img.save(out_path, format='PNG')  # or JPEG, WEBP\n"
            "\n"
            "Vary dimensions, bit depth, compression level. Use the template images as "
            "base material — do NOT synthesize raw bytes."
        )
    elif batch.lib_name and "sndfile" in batch.lib_name.lower():
        lines.append(
            "███ FORCED for libsndfile (audio library) ███\n"
            "Seeds must be valid audio files, not arbitrary bytes. A base audio template\n"
            "file is provided, use it as base material to build diverse seeds:\n"
            "\n"
            "  '/root/auto_harness/src/seed_generation/template_audio/base.ircam'\n"
            "\n"
            "Load its bytes and mutate specific regions (header fields, sample data, byte\n"
            "values) while keeping the file structurally valid. Example:\n"
            "\n"
            "  data = bytearray(open('/root/auto_harness/src/seed_generation/template_audio/base.ircam','rb').read())\n"
            "  data[POS] = ...   # mutate a header field / sample byte\n"
            "  open(out_path,'wb').write(data)\n"
            "\n"
            "Do NOT emit garbled random bytes — each seed must remain a parseable audio file."
        )
