...
def exec_distance_prog(dot, targets, out, names, cg_distance=None,
    ...
    """
    # try:
    #     if dot.name == "cfg.fill_buffer.dot" or dot.resolve() == TARGET.resolve():
...
def calculating_distances(args):
    dot_files = args.temporary_directory / DOT_DIR_NAME if not args.cfg_dot_files_path else Path(args.cfg_dot_files_path)
    # bbcalls = args.temporary_directory / "BBcalls.norm.txt"
    # bbnames = args.temporary_directory / "BBnames.norm.txt"
    # bbtargets = args.temporary_directory / "BBtargets.norm.txt"
    ...
    callgraph = dot_files / CALLGRAPH_NAME if not args.callgraph_path else Path(args.callgraph_path)
    ...
    def calculate_cfg_distance_from_file(cfg: Path):
        # try:
        #     if cfg.name == "cfg.fill_buffer.dot" or cfg.resolve() == TARGET.resolve():
        #         if debugpy is not None:
        #             debugpy.breakpoint()
        #         else:
        #             breakpoint()
        ...
        elif bn.startswith(".") and bn.endswith(".dot"):
            name = bn[1: -len(".dot")]
        else:
            core = bn[:-4] if bn.endswith(".dot") else bn
            name = core.split('.')[-1]
    ...
    with ThreadPoolExecutor(max_workers=mp.cpu_count()) as executor:
        results = executor.map(calculate_cfg_distance_from_file,
                               dot_files.glob(f"{args.cfg_glob_format}"))
...
def is_path_to_file(path):
    p = Path(path)
    if not p.exists():
        raise ArgTypeErr("path or doesn't exist")
    if not p.is_file():
        raise ArgTypeErr("not a file")
    return p
# ----
...
def main():
    ...
    parser.add_argument("--skip-cg",
                        action="store_true",
                        default=False,
                        help="Skip callgraph construction")
    parser.add_argument("--cfg-glob-format",
                        default="cfg.*.dot",
                        help="Format string for globbing CFG dot files. ")
    parser.add_argument("--callgraph-path",
                        type=is_path_to_file,
                        help="Path to callgraph dot file.When you didn't skip, you should not set this.")
    parser.add_argument("--cfg-dot-files-path",
                        type=is_path_to_dir,
                        help="Path to directory containing the CFG dot files. ")
    args = parser.parse_args()

    # Additional sanity checks
    if args.skip_cg and not args.cfg_dot_files_path:
        parser.error("When skipping CG construction, you must provide the "
                     "path to the CFG dot files via --cfg-dot-files-path.")
    
    if args.cfg_glob_format and not args.cfg_dot_files_path:
        parser.error("When providing a glob format for CFG dot files, you "
                     "must provide the path to the CFG dot files via "
                     "--cfg-dot-files-path.")
        
    binaries = list(args.binaries_directory.glob("*.0.0.*.bc"))
    if len(binaries) == 0 and not args.callgraph_path:
            parser.error("Couldn't find any binaries in folder "
                        f"{args.binaries_directory}.")
    #check the cfg.dot files for a specific fuzzer
    if args.fuzzer_name:
    ...
    if args.cfg_glob_format:
        tmp = args.cfg_dot_files_path.glob(f"{args.cfg_glob_format}")
        if len(list(tmp)) == 0:
            parser.error(f"Couldn't find CFG files matching {args.cfg_glob_format} "
                            f"in folder {args.cfg_dot_files_path}.")

    STEP = get_resume(args)

    if args.skip_cg and STEP < 1:
        STEP = 1

    if not args.skip_cg and STEP < 1:
        construct_callgraph(args, binaries)
    
    dot_files = args.temporary_directory / DOT_DIR_NAME
    cg_default = dot_files / CALLGRAPH_NAME
    cg_path = Path(args.callgraph_path) if args.callgraph_path else cg_default
    if args.skip_cg and not cg_path.exists():
        parser.error(f"Couldn't find callgraph at {cg_path}.")
