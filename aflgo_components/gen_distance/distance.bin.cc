...
typedef bo::adjacency_list<bo::vecS, bo::vecS, bo::directedS, Vertex, Edge,
                           graph_p>
    graph_t;
...
static inline std::string normalize_target(const std::string &name) {
  std::string s = name;
  bo::trim(s);
  if (!s.empty() && s.back() != ':') {
    s.push_back(':');
...
static inline std::string node_name(const std::string &name) {
  if (is_cg) {
    ...
    std::string s = normalize_target(name);
    return "{" + s + "}";
  }
}
...
inline void init_distances_from(const graph_t &G, vertex_desc from,
                                std::vector<int> &dists) {
  auto dist_pmap =
      bo::make_iterator_property_map(dists.begin(), get(bo::vertex_index, G));
  auto vis =
      bo::make_bfs_visitor(bo::record_distances(dist_pmap, bo::on_tree_edge()));
  bo::breadth_first_search(G, from, bo::visitor(vis));
}

void distance(const graph_t &G, const std::string &name,
              const std::vector<vertex_desc> &targets, std::ofstream &out,
      ...
      for (auto &bb_d_entry : bb_distance) {
        ...
        for (auto t : find_nodes(G, bb_d_entry.first)) {
          ...
          if (shortest == 0 and n != t)
            continue; // not reachable
          di += 1.0 /
                (1.0 + 10 * bb_d_entry.second + static_cast<double>(shortest));
          ++ii;
        }
        if (ii != 0) {
...
std::vector<vertex_desc> cg_calculation(graph_t &G,
...
std::vector<vertex_desc>
cfg_calculation(graph_t &G, std::ifstream &targets_stream,
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    desc.add_options()("help,h", "produce help message")(
        ...
        "graph.")("targets,t", po::value<std::string>()->required(),
                  "Path to file specifying Target"
                  " nodes.")("out,o", po::value<std::string>()->required(),
                             "Path to output file containing "
                             "distance for each node.")(
        "names,n", po::value<std::string>()->required(),
        "Path to file containing name for"
        " each node.")("cg_distance,c", po::value<std::string>(),
                       "Path to file containing call graph "
                       "distance.")("cg_callsites,s", po::value<std::string>(),
                                    "Path to file containing mapping "
  ...
  boost::ref_property_map<graph_t *, std::string> gname(
      get_property(graph, bo::graph_name));
  ...
  is_cg = get_property(graph, bo::graph_name).find("Call graph") !=
          std::string::npos;
  ...
  unordered_map<std::string, double>
      bb_distance; // describe the reachablility to the targets,and the
                   // corresponding shortest distance to the targets
  // the cfg which is dealing with may not be the one that contains the target,
  // so we need this "map" to describe the cross-function distance

  if (is_cg) {
    targets = cg_calculation(graph, targets_stream);
  } else {
    if (not vm.count("cg_distance")) {
      cerr << "error: the required argument for option '--cg_distance' is "
              "missing\n";
      exit(1);
    }
    if (not vm.count("cg_callsites")) {
      cerr << "error: the required argument for option '--cg_callsites' is "
              "missing\n";
      exit(1);
    }
    std::ifstream cg_distance_stream =
        open_file(vm["cg_distance"].as<std::string>());
    std::ifstream cg_callsites_stream =
        open_file(vm["cg_callsites"].as<std::string>());

    std::vector<std::string> splits;
    bo::algorithm::split(splits, vm["dot"].as<std::string>(),
                         bo::is_any_of("."));
    ;
