...
#if AFLGO_IMPL
    fn = alloc_printf("%s/hangs/id:%06llu,%llu,%s", out_dir, unique_hangs,
                      get_cur_time() - start_time, describe_op(0));
#else
    ...
    fprintf(f,
            ...
            (qemu_mode || dumb_mode || no_forkserver || crash_mode ||
             persistent_mode || deferred_mode)
                ? ""
                : "default",
            orig_cmdline, slowest_exec_ms, cur_distance, max_distance,
            min_distance);
#endif // AFLGO_IMPL
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fflush(plot_file);
    ...
    fprintf(plot_file,
            "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, "
            "%0.06f, %0.06f, %0.06f\n",
            get_cur_time() / 1000, (queue_cycle ? queue_cycle - 1 : 0),
            current_entry, queued_paths, pending_not_fuzzed, pending_favored,
            bitmap_cvg, unique_crashes, unique_hangs, max_depth, eps,
...
static u32 calculate_score(struct queue_entry *q) {
  ...
  // TODO Substitute functions of exp and log with faster bitwise operations on
  // integers
  ...
  if (q->distance > 0) {
    ...
    double normalized_d = 0; // when "max_distance == min_distance", we set the
                             // normalized_d to 0 so that we can sufficiently
                             // explore those testcases whose distance >= 0.
    if (max_distance != min_distance)
      normalized_d =
          (q->distance - min_distance) / (max_distance - min_distance);
      ...
      case 13:
        ...
        if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
          ...
          if (actually_clone)
            ...
            memset(new_buf + clone_to, UR(2) ? UR(256) : out_buf[UR(temp_len)],
                   clone_len);
...
/* ignore errors */
    ...
    fprintf(
        ...
        "max_depth, execs_per_sec, cur_distance, max_distance, min_distance\n");
#endif // AFLGO_IMPL
}
...
static void check_crash_handling(void) {
  ...
  if (!getenv("AFL_I_DONT_CARE_ABOUT_MISSING_CRASHES"))
    ...
    if (read(fd, &fchar, 1) == 1 && fchar == '|') {
      ...
      SAYF("\n" cLRD "[-] " cRST "Hmm, your system is configured to send core "
           "dump notifications to an\n"
